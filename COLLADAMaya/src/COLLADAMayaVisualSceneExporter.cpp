                            ...
                            else
                            {
                                ...
                                if( typeName == "color" )
                                {
...
#else//1
                                    {
                                        ...
                                        MPlug colorSetPlug = fnMesh.findPlug( ATTR_COLORSET );
                                        uint numElements = colorSetPlug.numElements();
                                        uint realIndex=0;
                                        for ( ; realIndex<numElements; ++realIndex )
                                        {
                                            // get uvSet[<index>] and uvSet[<index>].uvSetName
                                            MPlug colorSetElememtPlug = colorSetPlug.elementByPhysicalIndex ( realIndex );
                                            MPlug colorSetNamePlug = colorSetElememtPlug.child ( 0 );

                                            // get value of plug (uvSet's name)
                                            MString colorSetNamePlugValue;
                                            colorSetNamePlug.getValue ( colorSetNamePlugValue );

                                            if ( setName == colorSetNamePlugValue )
                                                break;
                                        }
